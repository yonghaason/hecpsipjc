...
LAN. For 32-bit payloads, where a correct integer inner product needs an
84-bit arithmetic space, we run the payload path over an RNS of two 42-bit
primes; our cost grows by $1.56\times$ while the quadratic baseline grows by
$4.65\times$.
\end{abstract}
...
bits 이상이어야 하며 이것이 단일 RLWE plaintext modulus를 넘을 때 RNS로
처리한다. 각 문제는 개별적으로는 알려진 기법으로 풀리지만, 한 프로토콜 안에서
...
정수 inner product 설정(\Cref{sec:integer-ip})은 42-bit 잔여류 두 개와
coefficient modulus \(\{60,45,20\}\)을 사용하며, 실제로 쓰이는 앞 두 소수의
합 105 bits로 128-bit 안전성이 유지되는 이유는 \Cref{sec:integer-ip}에 적었다.
실험에 사용한 파라미터는
...
설정에서도 제안 프로토콜의 통신 효율성은 유지되었다. 42-bit 잔여류 두 개의
RNS로 payload 경로만 두 배가 되어 $n=2^{20}$에서 통신량은 218.010\,MB에서
