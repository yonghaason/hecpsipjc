...
42-bit plaintext에는 SEAL의 128-bit security table이 $N=4096$에 허용하는
109-bit 총합보다 큰 coefficient modulus가 필요하다. 본 프로토콜은
key switching을 수행하지 않으므로 coefficient modulus의 마지막 소수(SEAL의
key-switching prime)는 사용되지 않으며, 실제로 쓰이는 앞 두 소수의 합만
109 bits 이내로 유지하면 된다. 이에 security level 검사를 해제하고
coefficient modulus를 $\{60,45,20\}$으로 둔다. Uniformly random한 share를
...
coefficient modulus \(\{60,45,20\}\)을 사용하며, 이 경우 SEAL의 security
table 검사를 해제한 이유는 \Cref{sec:integer-ip}에 적었다.
실험에 사용한 파라미터는
...
Coefficient modulus bit-sizes & \(\{60,45,20\}\) bits, security check off \\
\bottomrule
\end{tabular}
\end{table}
