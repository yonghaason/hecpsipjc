...
\noindentparagraph{\textbf{Why not a ciphertext--ciphertext construction.}}
...
plaintext--ciphertext multiplication 한 번만 남고, 그 차이가 $q\approx2^{84}$의
작은 ciphertext modulus를 가능하게 한다. 이에 B2A-OT 기반 구성을 최종 프로토콜로
채택하였다.
...
\(p\approx 2^{32}\), ciphertext modulus는 \(q\approx 2^{84}\)로 설정하였다.
Homomorphic evaluation은 암호문과 평문 사이의 multiplication 한 번과 modulus
switching으로 구성되므로 이 정도의 \(q\)로 충분하다. Batching으로 하나의
ciphertext에 최대 \(N=4096\)개의 값을 packing하였다.
정수 inner product 설정(\Cref{sec:integer-ip})에서는 plaintext modulus로
42-bit 소수 두 개(\(p_1,p_2\approx 2^{42}\))를, ciphertext modulus로
\(q\approx 2^{105}\)를 사용하였다.
실험에 사용한 파라미터는
...
\subsection{Comparison: Modular Inner Product}
...
\noindentparagraph{\textbf{Breakdown.}}
...
\subsection{Comparison: Integer Inner Product}
...
\Cref{sec:performance-comparison,sec:integer-ip-exp},
