...
The fastest recent constructions build it from circuit-PSI over OKVS. We
make two contributions. First, we show that the PJC implementation behind
the reported numbers of a recent OKVS-based protocol reuses a single global
mask across all payload shares, which lets the sender recover the XOR of any
two of the receiver's shares; the reported performance was therefore
obtained at a weaker security level than the one claimed, and is not
comparable like for like. Second, we give a PJC protocol whose cost is
linear in the payload width $\ell$ rather than quadratic. We lift
circuit-PSI from $\mathbb{F}_2$ to $\mathbb{Z}_p$ (AF-CPSI) so that its
output shares feed a BGV inner product directly, prove the construction
secure against semi-honest adversaries, and identify and close a leakage in
the $\mathbb{Z}_p$-to-bitstring encoding that the lift introduces. Against
the best prior circuit-PSI-based PJC at $n=2^{20}$ in the 32-bit setting, our
protocol uses 32\% less communication and runs 60\% faster on a
LAN. When the inner product over 32-bit payloads is required as an exact
integer rather than modulo a fixed prime, our communication grows by $1.56\times$ while the quadratic baseline grows by
$4.65\times$.
\end{abstract}
