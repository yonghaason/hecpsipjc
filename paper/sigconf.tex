...
아래 \Cref{eq:redundant-encoding}의 redundant encoding으로 구성된다고
하자. 이때 $\prot{AF-CPSI}$\emph{(\Cref{fig:cpsi-zp-construction})}는
$(\func{OPRF},\func{PEQT},\func{OT})$-hybrid model에서 semi-honest
adversary에 대해 $\func{AF-CPSI}$를 securely realize한다.
\end{theorem}

\begin{proof}[Proof sketch]
기존 Circuit-PSI 증명에서 payload masking이 사용되는 부분은 두 곳이며,
두 경우 모두 one-time pad argument에 의존한다. AF-CPSI는 이 argument가
성립하는 group만을 바꾼다.

첫째, OKVS의 data component에서 기존 증명은 $w_i\xleftarrow{\$}\{0,1\}^\ell$에
대해 $D_i^{Y'}\oplus w_i$가 $\{0,1\}^\ell$ 위에서 uniform함을 사용한다.
AF-CPSI는 $w_i\xleftarrow{\$}\Z_p$에 대해 $u_i=D_i^{Y'}-w_i \bmod p$가
$\Z_p$ 위에서 uniform함을 사용한다. 둘째, B2A-OT에서 기존 증명은
$r\xleftarrow{\$}\{0,1\}^\ell$에 대한 $(r,\,v\oplus r)$의 uniformity를
사용하며, AF-CPSI는 $r\xleftarrow{\$}\Z_p$에 대한 $(r,\,v-r \bmod p)$의
uniformity를 사용한다. 두 경우 모두 $(\{0,1\}^\ell,\oplus)$를
$(\Z_p,+)$로 치환한 것에 지나지 않으므로, simulator와 hybrid의 구성은
기존 증명과 동일하다. OPRF masking과 OKVS의 encoding 및 decoding은
\Cref{sec:cpsi-zp}에서 기술하였듯 비트열 위의 연산으로 유지되므로 변경되지
않는다.

한편 두 group의 치환이 형식적이지 않은 지점이 하나 존재한다.
$u_i\in\Z_p$를 OKVS value에 담기 위해서는 이를 고정 길이 비트열
$\overline{u}_i$로 encoding해야 하는데, $p$가 $2$의 거듭제곱이 아니면
$\Z_p$ 원소의 $\lceil\log_2 p\rceil$-bit 표현은
$\{0,1\}^{\lceil\log_2 p\rceil}$ 위에서 uniform하지 않다.
교집합에 해당하는 bin에서 $\Snd$는 유효한 encoding을 복원하는 반면,
교집합에 해당하지 않는 bin에서는 pseudorandom한 비트열을 얻으므로,
$\Snd$는 복원한 값이 $[0,p)$를 벗어나는지를 확인하는 것만으로
확률 $(2^{\lceil\log_2 p\rceil}-p)/2^{\lceil\log_2 p\rceil}$로
해당 bin이 교집합에 속하지 않음을 알 수 있다. 이는 $b_i$에 대한
정보이므로 simulator가 재현할 수 없다.

따라서 AF-CPSI에서는 statistical security parameter $\lambda_s$에 대해
$L=\lceil\log_2 p\rceil+\lambda_s$로 두고, data component를
...
\right\}
\end{equation}
와 같이 redundant하게 encoding한다. $\Snd$는 복원한 $\overline{u}_i$에
대해 $w_i'=\overline{u}_i \bmod p$를 계산하며, 이때 임의의
$\overline{u}_i$가 유효하므로 위의 range check는 불가능하다.
또한 $u_i$가 $\Z_p$ 위에서 uniform할 때 $\overline{u}_i$는
$\{0,1\}^{L}$ 위의 uniform distribution과 statistical distance
$p/2^{L}\le 2^{-\lambda_s}$ 이내이므로, simulator는 해당 component를
uniform한 비트열로 대체할 수 있다. 나머지 hybrid는 기존 증명과 동일하다.
\end{proof}

\begin{remark}\label{rmk:encoding-cost}
\Cref{eq:redundant-encoding}은 OKVS data component의 길이를
$\lceil\log_2 p\rceil$에서 $\lceil\log_2 p\rceil+\lambda_s$로 늘리므로,
\Cref{sec:comm-analysis}의 $C_{\mathsf{OKVS}}$에
$3\cdot 1.3n\lambda_s$ bits가 추가된다.
\end{remark}

\subsection{Security of the Proposed PJC Protocol}\label{sec:security-pjc}

제안 PJC 프로토콜에서 $\Snd$는 BGV의 secret key $sk$를 보유하고,
$\Rcv$가 homomorphic evaluation을 수행한 암호문을 $\Snd$에게 전달한다.
따라서 $\Snd$는 수신한 암호문을 복호화할 수 있을 뿐 아니라, 복호화
과정에서 noise를 정확히 복원할 수 있다. 이로 인해 평문에 대한 masking만으로는
$\Rcv$의 입력을 보호할 수 없으며, 아래에서 보이듯 evaluation 결과에 대한
sanitization이 필요하다.

\begin{theorem}\label{thm:pjc}
BGV가 IND-CPA secure하고, evaluation 결과에 대해 statistical
parameter $\lambda_s$의 noise flooding이 적용된다고 하자. 이때
$\prot{PJC}$\emph{(\Cref{fig:our-pjc})}는 $\func{AF-CPSI}$-hybrid
model에서 semi-honest adversary에 대해 $\func{PJC}$를 securely
realize한다.
\end{theorem}
...
다음으로 noise를 분석한다. $C_j^X$의 noise를 $e_j$라 하면,
$\mathsf{PlainMult}$와 $\mathsf{PlainAdd}$의 정의에 의해 $C^F$의 noise는
$\sum_{j\in[n_c]} \mathbf{A}_j^R\cdot e_j$에 modulus switching의 rounding
error를 더한 값이 된다. $\Snd$는 $sk$를 보유하므로 복호화 과정에서 이
noise를 정확히 복원할 수 있고, $\{e_j\}$는 $\Snd$ 자신이 생성한
encryption randomness로부터 계산할 수 있다. 따라서 sanitization이 없다면
$\Snd$는 $\sum_j \mathbf{A}_j^R\cdot e_j$를 얻으며, 특히 $n_c=1$이고
$e_1$이 $R_q$에서 invertible한 경우 $\mathbf{A}_1^R=\mathbf{a}^R$를 그대로
복원할 수 있다. 이 경우 $a_i^S+a_i^R=b_iD_i^{Y'}$로부터 $\Rcv$의 payload와
교집합이 모두 노출된다. 그러므로 $\Rcv$는 $C^F$를 전송하기 전
$C^F\leftarrow C^F+\mathsf{BGV.Enc}(pk,\mathbf{0};E)$와 같이 기존 noise
bound보다 $2^{\lambda_s}$배 큰 noise $E$를 더하는 noise flooding을
적용해야 하며, 이때 $C^F$의 noise 분포는 $\mathbf{a}^R$ 및 $\mathbf{r}$와
statistical distance $2^{-\lambda_s}$ 이내에서 독립이 된다.
...
$\tilde{C}\leftarrow\mathsf{BGV.Enc}(pk,\tilde{\mathbf{m}})$를 계산한 뒤
동일한 noise flooding을 적용하여 view에 포함시킨다.

$\mathsf{H}_0$을 실제 view라 하고, $\mathsf{H}_1$을 $C^F$의 평문만
$\tilde{\mathbf{m}}$으로 대체한 것이라 하자. 위의 평문 분석에 의해 두
분포는 동일하다. $\mathsf{H}_2$는 $C^F$를 $\tilde C$로 대체한 것이며,
noise flooding에 의해 두 noise 분포의 statistical distance는
$2^{-\lambda_s}$ 이하이다. 따라서
$\mathsf{H}_0\stackrel{c}{\equiv}\mathsf{H}_2$이고, $\mathsf{Sim}_\Snd$의
출력은 실제 view와 구분 불가능하다.
\end{proof}

\begin{remark}\label{rmk:flooding-cost}
\Cref{thm:pjc}의 noise flooding은 ciphertext modulus $q$를 약
$\lambda_s$ bits만큼 크게 설정할 것을 요구한다.
\Cref{sec:experimental-setup}의 파라미터는 $N=4096$에서 $\log q=102$
bits로 SEAL이 제공하는 $128$-bit security bound인 $109$ bits에 근접해
있으므로, flooding을 적용하려면 $N=8192$로 옮겨야 한다.
\end{remark}
