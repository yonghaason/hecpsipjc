...
it underlies ad-conversion measurement and similar joint analytics. Its cost
is dominated by how the payload values are handled once the intersection is
identified, and circuit-PSI-based constructions have paid a price quadratic
in the payload width $\ell$. A recent protocol (NDSS~'26) reported
bringing this to $O(\ell)$.
...
per party our protocol uses 32\% less communication and runs 60\% faster on
a LAN; when the inner product is required as an exact integer rather than
modulo a fixed prime, the gap widens to $4.4\times$ less communication.
\end{abstract}
...
실험에서 $n=2^{20}$의 32-bit 설정일 때 \cite{KLS26} 대비 통신량은 32.4\%, LAN
실행 시간은 59.5\% 감소한다. 32-bit payload에 대한 정수 inner product를 계산하는
설정에서는
...
Payload의 bit-length를 $\ell$로, payload에 대한 연산이 수행되는 arithmetic
space $\mathbb{Z}_P$의 bit-length를 $L=\lceil\log_2 P\rceil$로 표기하며, 두
값은 일반적으로 다르다.
...
Set size를 $n$이라 할 때 (32-bit 설정에서는 $L=32$이다),
...
20.900\,MB에서 14.150\,MB로 약 32.3\% 감소하였으며, LAN
환경의 실행 시간은 1.740\,s에서 0.740\,s로 약 57.5\%
감소하였다. Set size가 \(2^{20}\)인 경우에도 통신량은
322.622\,MB에서 218.010\,MB로 약 32.4\%, LAN 실행 시간은
28.105\,s에서 11.389\,s로 약 59.5\% 감소하였다. 또한
...
향후 연구로는 정수 inner product 설정의 WAN 측정, 전체 비용의 대부분을
차지하는 AF-CPSI 단계의 최적화, 그리고 실제 지연 시간이 있는 네트워크에서의
평가가 남아 있다.
