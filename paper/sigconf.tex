...
(\Cref{sec:comm-analysis}), 정수 inner product를 위한 long-item 설정
(\Cref{sec:long-items})과 채택하지 않은 대안(\Cref{sec:ctct})을 다룬다.
...
\Cref{sec:long-items}의 RNS 변형은 같은 프로토콜을 잔여류별로 독립 실행한
...
경우에 $L$을 어떻게 잡아야 하는지는 \Cref{sec:long-items}에서 다룬다.
...
\subsection{Handling Long Items}\label{sec:long-items}
...
\begin{table}[h]
\centering
\caption{Long-item setting: $\ell=32$-bit payloads, $L=84$-bit arithmetic space, LAN.}
\label{tab:pjc-long}
...
\midrule
\multicolumn{2}{l}{\textit{Long-item setting (\Cref{sec:long-items})}} \\
...
long-item 설정의 실측 비교는 \Cref{sec:long-items}에 제시하였다.
...
정수 inner product를 위해 $L=84$-bit arithmetic space가 필요한 long-item
...
향후 연구에서는 long-item 설정을 WAN 환경까지 확장하여 측정하고, 전체 실행 시간과
