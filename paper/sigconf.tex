...
이에 본 논문은 마스킹 값을 사용하는 프로토콜을 새롭게 교정하고 $\mathbb{Z}_p$ 위에서 arithmetic share 형태로 출력되도록 변형하여 교집합 원소를 식별한 후, 동형암호를 이용하여 교집합 원소에 대응되는 값들의 내적을 계산하는 프로토콜을 설계하였고, 실험을 통해 성능이 개선되었음을 확인하였다.
또한 정수 inner product에 필요한 arithmetic space가 단일 RLWE plaintext modulus를
넘는 경우를 위해 payload 경로를 RNS로 확장하고, 그 비용을 실측으로 보인다.
$n=2^{20}$, $L=32$에서 \cite{KLS26} 대비 통신량은 32.4\%, LAN 실행 시간은 59.5\% 감소하며, 정수 inner product를 위한 $L=84$ 설정에서는 제안 프로토콜의 비용이 $1.56$배 증가하는 동안 \cite{KLS26}의 비용은 MultShare의 이차 증가로 약 $4.65$배 증가한다.
