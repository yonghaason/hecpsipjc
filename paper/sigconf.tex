...
이 두 연구는 PJC의 비용이 payload의 bit-length $\ell$에 어떻게 의존하는지에서
갈린다. \cite{KLS26}의 PSI-InnerProduct는 두 arithmetic share의 곱을 얻기 위해
한쪽 값을 bit decomposition한 뒤 각 bit마다 $\ell$-bit 메시지에 대한 OT를
수행하므로, 원소당 $O(\ell^2)$의 비용이 발생한다. 반면 \cite{LTS+26}은 동일한
계산을 원소당 $O(\ell)$에 수행할 수 있다고 보고하였다. $\ell$은 inner product의
대상이 되는 payload의 bit-length이므로, 이 차이는 payload가 커질수록 그대로
누적된다.
...
이러한 leakage를 허용하면 마스킹 과정에서 요구되는 보안 제약이 완화되므로, 보다 단순하고 효율적인 construction을 구성할 수 있다. 즉 \cite{LTS+26}이 보고한 효율성의 일부는 프로토콜 설계가 아니라 완화된 보안 목표에서 기인한다. 따라서 해당 수치를 $\mathcal{F}_{\mathrm{2PC}}$ 수준의 보안을 만족하는 PJC 프로토콜과 직접 비교하는 것은 타당하지 않으며, \Cref{sec:comm-analysis}에서는 동일한 보안 목표를 전제로 한 비교를 제시한다.
...
앞에서 $u_i$를 고정 길이 비트열 $\overline{u}_i$로 변환한다고만 서술하였는데,
이 변환은 표현 형식을 맞추기 위한 형식적인 단계로 보이지만 실제로는 보안에
영향을 준다. $p$가 $2$의 거듭제곱이 아닌 경우 $\Z_p$ 원소의
$\lceil\log_2 p\rceil$-bit 표현은 가능한 비트열 전체를 채우지 못하고
$[0,p)$에 해당하는 부분만을 차지하기 때문이다.
...
본 논문이 사용하는 $p=4{,}294{,}475{,}777$에서 이 값은 bin당 약 $2^{-13.1}$로,
목표하는 statistical security와 비교하면 무시할 수 없는 수준이다.
...
\rho_i\xleftarrow{\$}
\left[\,0,\;
\left\lfloor\frac{2^{L}-1-u_i}{p}\right\rfloor
\right]
\end{equation}
와 같이 구성한다. $\Snd$는 복원한 값에 대해
$w_i'=\overline{u}_i \bmod p$를 계산하므로 correctness는 그대로 유지된다.

이 encoding에서는 모든 $L$-bit 비트열이 어떤 잉여류엔가 대응되므로 위와 같은
range check 자체가 성립하지 않는다. 또한 $u_i$가 $\Z_p$ 위에서 uniform할 때
$\overline{u}_i$의 분포는 $\{0,1\}^{L}$ 위의 uniform distribution과
statistical distance $p/2^{L}\le 2^{-\lambda_s}$ 이내이므로, 교집합에
해당하지 않는 bin에서 $\Snd$가 얻는 pseudorandom한 값과 구분되지 않는다.
이 성질은 \Cref{thm:afcpsi}의 증명에서 사용된다.
...
본 절에서는 제안 프로토콜의 보안성을 분석한다.
\Cref{sec:security-model}에서 보안 모델을 정의하고,
\Cref{sec:security-afcpsi}에서 AF-CPSI가 $\func{AF-CPSI}$를 realize함을
보인 뒤, \Cref{sec:security-pjc}에서 제안 PJC 프로토콜이
