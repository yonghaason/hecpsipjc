...
$L=\lceil\log_2 p\rceil+\lambda_s$로 두고, $\Rcv$는 $u_i$ 자체가 아니라
$u_i$와 같은 잉여류에 속하는 $L$-bit 값 중 하나를 무작위로 선택하여
\begin{equation}\label{eq:redundant-encoding}
\overline{u}_i=u_i+p\cdot\rho_i,
\qquad
\rho_i\xleftarrow{\$}
\left[\,0,\;
\left\lfloor 2^{L}/p \right\rfloor
\right)
\end{equation}
와 같이 구성한다. $\Snd$는 복원한 값에 대해
$w_i'=\overline{u}_i \bmod p$를 계산하므로 correctness는 그대로 유지된다.

모든 $L$-bit 비트열은 어떤 잉여류에 속하므로, 이 encoding에서는 위와 같은
range check 자체가 성립하지 않는다. 또한 $u_i$가 $\Z_p$ 위에서 uniform하면
$\overline{u}_i$는 $[0,\,p\lfloor 2^{L}/p\rfloor)$ 위에서 uniform하고,
$\{0,1\}^{L}$ 위의 uniform distribution과의 statistical distance는
$\bigl(2^{L}-p\lfloor 2^{L}/p\rfloor\bigr)/2^{L}<p/2^{L}\le 2^{-\lambda_s}$이다.
...
보장하려면 $L=\lceil\log_2 p\rceil+\lambda_s+\log_2 n'$로 두면 된다.
...
\begin{figure}[h]
    ...
    \begin{enumerate}
        ...
        $
            u_i=D_i^{Y'}-w_i\pmod p.
        $
        Then, $\mathcal{R}$ encodes $u_i$ into an $L$-bit string
...
\eqref{eq:redundant-encoding}에 의해 $\overline{u}_i$의 분포는 $\{0,1\}^{L}$
...
이하에서는 payload의 bit-length를 $\ell$, 연산이 수행되는 arithmetic
space $\Z_P$의 bit-length를 $L=\lceil\log_2 P\rceil$로 구분하여 표기한다.
앞 절까지의 실험은 $\ell=16$, $L=32$이며, 출력은 $\Z_P$ 위의 값, 즉
\emph{modular} inner product $\sum_{x_i=y_j} D_i^X D_j^Y \bmod P$이다.
이는 그 자체로 잘 정의된 functionality이고, 비교 대상인 \cite{KLS26} 역시
동일한 $L$의 ring 위에서 같은 값을 계산한다. 정수 inner product가 필요한
경우에 $L$을 어떻게 잡아야 하는지는 \Cref{sec:integer-ip}에서 다룬다.

Arithmetic space의 bit-length를 $L$, set size를 $n$이라 할 때,
...
위해서는 같은 $L=84$ ring이 필요하고, MultShare가 $L$에 대해 이차이므로 비용은
$L=32$ 대비 약 $(84/32)^2$배가 된다. \cite{KLS26}의 구현은 $\Z_{2^{32}}$ 위에
