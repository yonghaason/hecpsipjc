...
이러한 개념들 중 practice에서 매우 유용한 확장 개념으로, PJC(Private Join and Compute)~\cite{LPR+21}가 주목받고 있다. PJC는 두 참여자의 입력 집합에서 교집합 원소에 대응되는 값들의 내적(inner product)을 안전하게 계산하는 two-party 프로토콜로, 통계 분석, 광고 효과 분석, 추천 시스템, 머신러닝 등 다양한 데이터 분석 분야~\cite{IKN+20}에서 활용 등이 논의되고 있는 중요한 암호 프로토콜이다.
...
첫번째로, we present a cryptanalysis of PJC protocol proposed in \cite{LTS+26}. 송신자의 데이터를 비밀리에 전달하는 과정에서 하나의 마스킹 값을 전체에 적용하여 수신자가 원본 데이터와 마스킹 값 간의 차분을 알 수 있다는 문제가 존재한다는 것을 발견하였다. 이 경우 실제 데이터 값은 알 수 없더라도 데이터 간의 차분을 통해 수신자가 원본 데이터를 알아내지 않고도 원소 간의 상대적인 관계나 데이터의 분포와 같은 부가적인 정보를 추론할 수 있게 된다. 

두번째로, $O(\ell)$을 만족하는 new PJC protocol을 제시한다.
현재 Circuit-PSI는 XOR이 출력되도록 filed를 설정하지만, 이는 PJC 단계에서 사용하는 동형암호(Homomorphic Encryption, HE)와 연산의 filed가 호환되지 않아 PSI의 결과를 바탕으로 PJC의 해시함수를 시행하는 데에 어려움이 존재하였다.
...
본 논문에서 Sender $S$와 Receiver $R$은 프로토콜에 참여하는 두 참여자이며, $S$와 $R$이 보유한 식별자 집합을 각각 $X=\{x_1,\ldots,x_n\}$과 $Y=\{y_1,\ldots,y_n\}$으로 나타내고, $n=|X|, |Y|$로 둔다.
...
Oblivious Transfer(OT)은 참여자 $\mathcal{S}$와 $\mathcal{R}$이 각각 두 개의 비밀정보를 가지고 있을 때, $\mathcal{S}$가 1-out-of-2 OT에 의해 두 개의 비밀정보를 불확정 전송하는 상황에서 $\mathcal{R}$은 $\mathcal{S}$의 정보 중 1/2만 알게 되고, $\mathcal{S}$는 $\mathcal{R}$이 어떤 정보를 알고 있는 지 알 수 없게 하는 프로토콜이다. 이는 $\mathcal{S}$가 n개의 비밀을 가지고 있을 때의 상황을 가정하여 $\mathcal{R}$은 $\mathcal{S}$의 어떤 정보를 가졌는지 알 수 없고, $\mathcal{S}$는 $\mathcal{R}$이 어떤 정보를 가지고 있는지 알 수 없도록 확장할 수 있다. 이에 대한 ideal functionality $\mathcal{F}_{\mathrm{OT}}$는 figure~\ref{fig:ot-1-n-ideal-functionality}에 나타낸다.
...
\begin{figure}[h]
    ...
    \label{fig:ot-1-n-ideal-functionality}
\end{figure}
...
\subsection{Oblivious Pseudorandom Function}
...
\subsection{Oblivious Key-Value Store}
...
난수 $\mathcal{R}$을 입력받아 해당 키에 대응하는 값 \(v\)를 출력한다.
\[
...
\section{Recap of Circuit Private Set Intersection}
This section에서는, Our contribution--cryptanalysis and construction--을 one stroke로 이해할 수 있는 주요 background인, circuit-based PSI~\cite{PSTY19, RS21}라는 개념을 정리한다. 
...
B2A-OT는 Section~2.2에서 설명하였듯, membership bit $b_i$와 입력값 $v$에 대해
...
\section{Cryptanalysis of \cite{LTS+26}}
...
\cite{LTS+26}에서는 Base OT 기반 연산을 통해 2.6절에서 정의된 Circuit-PSI의 Step~1을 수행하여
...
먼저 $\mathcal{R}$는 하나의 random global mask $g$를 sample한다.
...
&\alpha^S+\alpha^R
    =\sum_i b_i \cdot (\omega_i \oplus w'_i) \cdot D^{X'},\\
\textsf{Y Sum:}\qquad
&\beta^S+\beta^R
    =\sum_i b_i \cdot D_i^{X'},\\
\textsf{Mask Cross:}\qquad
&\gamma^S+\gamma^R
    =g\sum_i b_i \cdot \beta^S.
\end{aligned}
\]

이후 $\mathcal{R}$는 자신이 보유한 share와 global mask $g$를 이용하여
\[
\delta
:=
...
본 논문에서는 Section~3.2에서 서술한 문제점과,
기존 Circuit-PSI construction에서 XOR 기반 share를 출력하는 단계로 인해 ${Z_P}$ 위에서 연산되는 동형암호의 message space와 직접 결합하기 어렵다는 문제점을 해결한 새로운 Circuit-PSI의 Arbitrary Field generalization, dubbed AF-CPSI를 제안한다.

Section~4.1에서는 AF-CPSI를 구성하기 위해 기존 Circuit-PSI에서
수정되는 부분을 설명하고, 
Section~4.2에서는 AF-CPSI에서 얻은 arithmetic share를
동형암호 연산에 입력하여 PJC를 수행하는 구체적인 방법을 설명한다.


\subsection{Circuit-PSI over arbitrary fields}\label{sec:cpsi-zp}
AF-CPSI는 Section~2.6에서 설명한 기존 Circuit-PSI의 두 단계에서 사용되는
...
Section~2.6의 Figure~\ref{fig:cpsi-construction}에서 설명한
...
Section~2.6에서 설명한 기존 Circuit-PSI와 동일하게 수행한다.
...
component에 각각 적용해야 한다. 이는 두 번에 B2A-OT로 구성된다.
...
$M$의 모든 slot의 합을 $M'$이라 하면 다음과 같이 표현할 수 있다.
\[
M'
=
\sum_{i=1}^{n'}
...
\noindentparagraph{\textbf{Ct-Ct Mult version.}}
...
우선, $\mathcal{R}$은 $\mathcal{R}$의 Boolean share와 data component를
...
추가로 수행된다. 이에 따라 전체 homomorphic evaluation의 multiplicative depth가 증가하고, ciphertext component의
수를 증가로 인해 relinearization이 필요하다. 또한, 이러한 동형 곱셈 이후에도 올바른 복호화를 보장하려면 충분한 noise budget을 확보할 수 있도록 BGV의 ciphertext modulus
...
LAN 환경은 별도 네트워크 제한 없는 localhost로, WAN 환경은 linus tc를 이용하여 bandwidth를 각각 100\,Mbps와 10\,Mbps로 제한하여 구성하였다.
...
AF-CPSI의 구성에는 Vole-PSI 구현을 기반으로 하며,
...
$\ell = 32$-bit로 평가하였다.
반면 \cite{CHI+23}은 공개 구현이 제공되지 않아 원 논문에서 보고된 figures를 제시한다. 심지어 이들은 $\ell = 23$을 사용하였기 때문에 communication의 경우 이들에게 유리하다고 볼 수 있음.
...
\begin{minipage}{0.88\linewidth}
\footnotesize
$^\dagger$ \cite{CHI+23}가 구현을 공개하지 않아서 runtime은 논문에 있는 LAN에서의 수치를 그대로 가져왔음. 
\end{minipage}
...
한편 \cite{CHI+23}은 23-bit setting를 사용하여 실험하였으므로,
...
\medskip
\noindentparagraph{\textbf{Communication analysis.}}
