...
the best prior circuit-PSI-based PJC at $n=2^{20}$ and a 32-bit arithmetic
space, our protocol uses 32\% less communication and runs 60\% faster on a
LAN. When the inner product is required as an exact integer rather than
modulo a fixed prime, which forces a wider arithmetic space, our
communication grows by $1.56\times$ while the quadratic baseline grows by
$4.65\times$.
\end{abstract}
...
엄밀히는 두 비용 모두 payload가 아니라 연산이 수행되는 arithmetic space의
bit-length $L$에 대한 것이지만, 정수 inner product에서는 $L=\Theta(\ell)$이므로
(\Cref{sec:integer-ip}) $\ell$에 대해서도 같은 차수이다.
...
실험에서 $n=2^{20}$, $L=32$일 때 \cite{KLS26} 대비 통신량은 32.4\%, LAN
실행 시간은 59.5\% 감소한다. 정수 inner product를 위한 $L=84$ 설정에서는
...
Set size를 $n$이라 할 때 (본 절의 실험 설정은 $\ell=16$, $L=32$이다),
...
Plaintext moduli (RNS) & two 42-bit primes, $L=84$ \\
Coefficient modulus bit-sizes & \(\{60,45,20\}\) bits (105 used, 128-bit) \\
\bottomrule
\end{tabular}
\end{table}
...
$\ell=16$-bit payload, $L=32$-bit arithmetic space로 평가하였다.
...
\begin{table}[h]
\centering
\caption{Performance comparison of PJC protocols under different network settings ($\ell=16$, $L=32$, modular inner product).}
...
\cite{KLS26} 및 제안 프로토콜의 $L=32$ setting과 동일한 조건에서
측정된 결과가 아니다. 따라서 해당 값은 기존 PJC 프로토콜의
communication cost를 보여주는 참고값으로 사용한다.
한편 정수 inner product에 필요한 $L=84$-bit arithmetic space를 사용하는
...
\subsection{Integer Inner Product Setting}
\label{sec:integer-ip-exp}
\Cref{sec:integer-ip}의 설정, 즉 $\ell=32$-bit payload에 대해 $L=84$-bit
arithmetic space에서 정수 inner product를 계산하는 경우를 LAN에서 측정하였다.
...
\begin{table}[h]
\centering
\caption{Integer inner product setting: $\ell=32$-bit payloads, $L=84$-bit arithmetic space, LAN.}
\label{tab:pjc-long}
...
\toprule
\textbf{Set Size} & \textbf{Protocol} & \textbf{Comm. (MB)} & \textbf{LAN (s)} \\
\midrule
\multirow{2}{*}{$2^{16}$}
& \cite{KLS26} & 93.534 & 2.678 \\
& \textbf{Ours} & 21.832 & 0.823 \\
\midrule
\multirow{2}{*}{$2^{20}$}
& \cite{KLS26} & 1499.286 & 43.045 \\
& \textbf{Ours} & 340.631 & 13.386 \\
\bottomrule
\end{tabular}
\vspace{2pt}
\begin{minipage}{0.95\linewidth}
...
Table~\ref{tab:pjc-long}의 결과는 \Cref{tab:pjc-breakdown}의 $L=32$ 설정과
...
정수 inner product를 위해 $L=84$-bit arithmetic space가 필요한
